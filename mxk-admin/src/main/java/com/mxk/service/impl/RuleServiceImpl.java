...
import com.mxk.constants.EnabledEnum;
import com.mxk.constants.MatchMethodEnum;
...
import com.mxk.mapper.RouteRuleMapper;
...
import com.mxk.transfer.AppRuleVOTransfer;
import com.mxk.transfer.RuleVOTransfer;
...
public class RuleServiceImpl implements RuleService {
    ...
    private void checkRule(RuleDTO ruleDTO) {
        ...
        if (!CollectionUtils.isEmpty(ruleMapper.selectList(wrapper))) {
            throw new MxkException("规则名称不能重复");
        }
        if (MatchObjectEnum.DEFAULT.getCode().equals(ruleDTO.getMatchObject())) {
            ...
            if (StringUtils.isEmpty(ruleDTO.getMatchKey()) || ruleDTO.getMatchMethod() == null
                ...
                throw new MxkException(MxkExceptionEnum.PARAM_ERROR);
            }
        }
        // check version
        ...
        if (CollectionUtils.isEmpty(list)){
            throw new MxkException("实例版本不存在");
        }
    }
